...
void protodec_initialize(struct demod_state_t *d, struct mysql_data_t *m)
{
	...
	if (strcmp(m->mysql_keepsmall,"yes") == 0 || strcmp(m->mysql_keepsmall,"YES") ==0 ){
	 d->keepsmall = 1;
	}
	else{
	 d->keepsmall = 0;
	...
	if (m->mysql_dbname[0] != 0) d->enable_mysql = 1;
	else d->enable_mysql = 0;
...
void protodec_getdata(int bufferlengde,struct demod_state_t *d)
{
	...
	if (bufferlengde <= (senlen*6)){
	    ...
	    if (bufferlengde % (senlen*6) != 0) sentences++;
	    };
	...
	do{
	 ...
	 nmea[k]=44;nmea[k+1]=48;nmea[k+2]=42;nmea[k+3]=48;nmea[k+4]=48;nmea[k+5]=0;
	 sentencenum++;
	 
	 // printout one frame starts here
	 //AIVDM,x,x,,, - header comes here first

	 nmea[0]=65;nmea[1]=73;nmea[2]=86;nmea[3]=68;nmea[4]=77;nmea[5]=44;
	 nmea[6]=48+sentences;nmea[7]=44;nmea[8]=48+sentencenum;nmea[9]=44;
	 ...
	 if (sentences > 1){
	   ...
	   nmea[10]=d->seqnr+48;nmea[11]=44;nmea[12]=44;
	   ...
	   if (sentencenum == sentences){
	      ...
	      nmea[k+1]=48+fillbits;
	       ...
	       nmea[10]=44;nmea[11]=65;nmea[12]=44;
	      }
	...
	//printf("!%s\r\n",nmea);
	 if (d->my_fd != -1){
	    int n = write(d->my_fd,d->serbuffer,strlen(d->serbuffer));
	    if (n < 0){
...
#ifdef HAVE_MYSQL
	   	sprintf(sqlquery,"insert into ais_nmea (time, message) values (%d,\"!%s\")",(int)tid,nmea);
	if(d->enable_mysql){
...
switch(type){
    case 1:
	   ...
	   printf("%d:  %09d %10f %10f %5f %5f %5i %5d %5d",type,mmsi,(float)latitude/600000,(float)longitude/600000,(float)coarse/10,(float)sog/10,rateofturn,underway,heading);
	 printf("  ( !%s )\r\n",nmea);
#ifdef HAVE_MYSQL
	   	sprintf(sqlquery,"insert into ais_position (time,mmsi,latitude,longitude,heading,coarse,speed) values (%d,%d,%.7f,%.7f,%.5f,%f,%f)",(int)tid,(int)mmsi,(float)latitude/600000,(float)longitude/600000,(float)heading,(float)coarse/10,(float)sog/10);

		sprintf(sqlquery2,"update ais_position set time=%d, latitude=%.7f,longitude=%.7f,heading=%f,coarse=%.5f,speed=%f where mmsi=%d",(int)tid,(float)latitude/600000,(float)longitude/600000,(float)heading,(float)coarse/10,(float)sog/10,(int)mmsi);
#endif
	}
       break;
	case 2:
	...
	if (d->skip_type[4] == 0){
	    ...
	    printf("%d:  %09d %d %d %d %d %d %d %f %f",type,mmsi,year,month,day,hour,minute,second,latit,longit);
	 printf("  ( !%s )\r\n",nmea);
#ifdef HAVE_MYSQL
	   	sprintf(sqlquery,"insert into ais_basestation (time,mmsi,longitude,latitude) values (%d,%d,%.7f,%.7f)",(int)tid,(int)mmsi,(float)latitude/600000,(float)longitude/600000);

	   	sprintf(sqlquery2,"update ais_basestation set time=%d,latitude=%.7f,longitude=%.7f where mmsi=%d",(int)tid,(float)latitude/600000,(float)longitude/600000,(int)mmsi);
...
//	printf("Length: %d\nWidth: %d\nHeight: %f\n",A+B,C+D,(float)height/10);
	printf("%d:  %09d %s %s %d %d %f",type,mmsi,name,destination,A+B,C+D,(float)height/10);
	 printf("  ( !%s )\r\n",nmea);
#ifdef HAVE_MYSQL

	   	sprintf(sqlquery,"insert into ais_vesseldata (time,mmsi,name,destination,height,A,B,C,D) values (%d,%d,\"%s\",\"%s\",%f,%d,%d,%d,%d)",(int)tid,(int)mmsi,name,destination,(float)height/10,(int)A,(int)B,(int)C,(int)D);
	   	
		sprintf(sqlquery2,"update ais_vesseldata set time=%d, name=\"%s\",destination=\"%s\",A=%d,B=%d,C=%d,D=%d,height=%f where mmsi=%d",(int)tid,name,destination,(int)A,(int)B,(int)C,(int)D,(float)height/10,(int)mmsi);
...
// delete too old entries;
	if (d->oldlimit > 0 ){
        sprintf(sqlquery3,"delete from ais_position where time<%d",(int)tid-d->oldlimit);
	DBG(printf("%s     Time now:%d\n",sqlquery3,(int)tid );)
        DBG(printf("Trying to delete oldies...");)
	if (mysql_query(&d->conn, sqlquery3))                	//try update data
	    {
	...
	case ST_STARTSIGN:                 
			...
			if(d->nstartsign >=7)
			{
				if (in[i] == 0)
				{
					...
					d->nstartsign = 0;
					d->antallenner = 0;
					memset(d->buffer,0,250*sizeof(char));
					d->bufferpos = 0;
				}
				else {
					protodec_reset(d);

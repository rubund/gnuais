...
int main(int argc, char *argv[])
{
	...
	buffer_l -= extra;
	if ((buffer =
	     (short *) malloc((buffer_l) * sizeof(short))) == NULL) {
	...
	printf
	    ("Starts demodulating and decoding AIS with sound data from file %s...",
	     argv[1]);
	if (mysql_data.mysql_dbname[0] != 0)
		printf(" (Saving to MySQL database \"%s\")",
		       mysql_data.mysql_dbname);
	printf("\n\n");
